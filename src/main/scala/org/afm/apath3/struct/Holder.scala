...
import org.afm.apath3.core.{Apath, Config}
...
abstract class Holder(config: Config, var root: Option[AnyRef] = None) extends Apath(config) {
  ...
  def get[R](expr: String) = super.get[R](root.get, expr)
  def doMatch(expr: String) = super.doMatch(root.get, expr)

  def setObject[R](root: AnyRef) = {
