...
class PathParser() {
  ...
  val arity: Map[String, Int] = Map("not" -> 1, "selector" -> 0, "text" -> 0, "rmatch" -> 1)
  ...
  def ensureFuncArity(size: Int, func: String): Unit = {
    ...
    val a: Int = arity(func)
    if (size - 2 != a) throw new RuntimeException(s"arity $a for func $func expected")
  }
  ...
  def buildExpr(sexpr: Seq[Any]): Expr = {
    ...
    func match {
      ...
      case "nodeBinding" => VarBind(asString(sub(sexpr, 1)).substring(1))
  ...
  private def buildFunc(sexpr: Seq[Any]) = {
    ...
    funcName match {
      ...
      case "selector" => Selector()
      case "text" => Text()
      case "rmatch" => RegexMatch() :+ buildExpr(sub(sexpr, 2))
      case _ => throw new RuntimeException(s"func '${asString(sexpr, 1)}' not implemented")
    }
  }
